...
import type { StandarPelayananItem } from "@/lib/api/types/beranda.types";
...
// Function untuk mendapatkan icon berdasarkan index
const getCardIcon = (index: number) => {
  const icons = [FileText, Shield, Award, Users, Settings, CheckCircle];
  ...
  <section class="bg-gray-50 py-16">
    ...
    <div class="container mb-6 flex items-center justify-between">
      <h2 class="text-2xl font-bold text-gray-800 uppercase">
        <span class="hidden border-b-4 border-yellow-600 pb-1 sm:block">Standar Pelayanan</span>
        <span class="block border-b-4 border-yellow-600 pb-1 sm:hidden">Pelayanan</span>
      </h2>
      <RouterLink
        :to="{ name: 'pelayanan-publik.standar' }"
    ...
    <template v-if="standarPelayanan.length">
      <!-- Aplikasi Terkait Grid -->
      <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3">
        <article
          v-for="(standarPelayanan, index) in standarPelayanan"
          :key="standarPelayanan.id"
        ...
        >
          ...
          <div class="absolute inset-0 overflow-hidden">
            <div
              ...
              style="background-image: url(/assets/images/backgrounds/bg-standar-pelayanan.png)"
            ></div>
          ...
          <div class="relative z-12 flex h-full flex-col p-4 sm:p-6">
            ...
            <div class="absolute top-3 right-3 sm:top-4 sm:right-4">
              <component :is="getCardIcon(index)" class="h-6 w-6 text-gray-800 group-hover:text-white sm:h-8 sm:w-8" />
            </div>

            <!-- Title -->
            <h3 class="mr-20 mb-2 line-clamp-2 text-xl font-bold text-gray-800 group-hover:text-white">
            ...
            >
              Standar pelayanan yang berkualitas untuk memberikan layanan terbaik kepada masyarakat.
            </p>
    ...
    <template v-else>
      <div class="rounded-lg bg-white p-8 text-center shadow-md">
        ...
        <p class="text-gray-600">Belum ada standar pelayanan tersedia</p>
      </div>
    </template>
  </section>
</template>
