<template>
  <div>
    <a :href="`https://www.facebook.com/sharer/sharer.php?u=${url}`" class="share-button facebook" target="_blank">
      <svg height="24" width="24" viewBox="0 0 24 24">
        <path
          fill="#ffffff"
          d="M17,2V2H17V6H15C14.31,6 14,6.81 14,7.5V10H14L17,10V14H14V22H10V14H7V10H10V6A4,4 0 0,1 14,2H17Z"
        />
      </svg>
      Bagikan ke Facebook</a
    >

    <a :href="`https://twitter.com/intent/tweet?url=${url}`" class="share-button twitter" target="_blank"
      ><svg height="24" width="24" viewBox="0 0 24 24">
        <path
          fill="#FFFFFF"
          d="M22.46,6C21.69,6.35 20.86,6.58 20,6.69C20.88,6.16 21.56,5.32 21.88,4.31C21.05,4.81 20.13,5.16 19.16,5.36C18.37,4.5 17.26,4 16,4C13.65,4 11.73,5.92 11.73,8.29C11.73,8.63 11.77,8.96 11.84,9.27C8.28,9.09 5.11,7.38 3,4.79C2.63,5.42 2.42,6.16 2.42,6.94C2.42,8.43 3.17,9.75 4.33,10.5C3.62,10.5 2.96,10.3 2.38,10C2.38,10 2.38,10 2.38,10.03C2.38,12.11 3.86,13.85 5.82,14.24C5.46,14.34 5.08,14.39 4.69,14.39C4.42,14.39 4.15,14.36 3.89,14.31C4.43,16 6,17.26 7.89,17.29C6.43,18.45 4.58,19.13 2.56,19.13C2.22,19.13 1.88,19.11 1.54,19.07C3.44,20.29 5.7,21 8.12,21C16,21 20.33,14.46 20.33,8.79C20.33,8.6 20.33,8.42 20.32,8.23C21.16,7.63 21.88,6.87 22.46,6Z"
        /></svg
      >Tweet</a
    >

    <a :href="`https://plus.google.com/share?url=${url}`" class="share-button google-plus" target="_blank"
      ><svg height="24" width="24" viewBox="0 0 24 24">
        <path
          fill="#000000"
          d="M23,11H21V9H19V11H17V13H19V15H21V13H23M8,11V13.4H12C11.8,14.4 10.8,16.4 8,16.4C5.6,16.4 3.7,14.4 3.7,12C3.7,9.6 5.6,7.6 8,7.6C9.4,7.6 10.3,8.2 10.8,8.7L12.7,6.9C11.5,5.7 9.9,5 8,5C4.1,5 1,8.1 1,12C1,15.9 4.1,19 8,19C12,19 14.7,16.2 14.7,12.2C14.7,11.7 14.7,11.4 14.6,11H8Z"
        /></svg
      >Bagikan ke Google+</a
    >

    <a :href="`https://pinterest.com/pin/create/button/?url=${url}`" class="share-button pinterest" target="_blank"
      ><svg height="24" width="24" viewBox="0 0 24 24">
        <path
          fill="#FFFFFF"
          d="M13.25,17.25C12.25,17.25 11.29,16.82 10.6,16.1L9.41,20.1L9.33,20.36L9.29,20.34C9.04,20.75 8.61,21 8.12,21C7.37,21 6.75,20.38 6.75,19.62C6.75,19.56 6.76,19.5 6.77,19.44L6.75,19.43L6.81,19.21L9.12,12.26C9.12,12.26 8.87,11.5 8.87,10.42C8.87,8.27 10.03,7.62 10.95,7.62C11.88,7.62 12.73,7.95 12.73,9.26C12.73,10.94 11.61,11.8 11.61,13C11.61,13.94 12.37,14.69 13.29,14.69C16.21,14.69 17.25,12.5 17.25,10.44C17.25,7.71 14.89,5.5 12,5.5C9.1,5.5 6.75,7.71 6.75,10.44C6.75,11.28 7,12.12 7.43,12.85C7.54,13.05 7.6,13.27 7.6,13.5A1.25,1.25 0 0,1 6.35,14.75C5.91,14.75 5.5,14.5 5.27,14.13C4.6,13 4.25,11.73 4.25,10.44C4.25,6.33 7.73,3 12,3C16.27,3 19.75,6.33 19.75,10.44C19.75,13.72 17.71,17.25 13.25,17.25Z"
        /></svg
      >Bagikan ke Pinterest</a
    >

    <a href="" class="share-button tumblr" target="_blank"
      ><svg height="24" width="24" viewBox="0 0 24 24">
        <path
          fill="#FFFFFF"
          d="M16,11H13V14.9C13,15.63 13.14,16 14.1,16H16V19C16,19 14.97,19.1 13.9,19.1C11.25,19.1 10,17.5 10,15.7V11H8V8.2C10.41,8 10.62,6.16 10.8,5H13V8H16M20,2H4C2.89,2 2,2.89 2,4V20A2,2 0 0,0 4,22H20A2,2 0 0,0 22,20V4C22,2.89 21.1,2 20,2Z"
        /></svg
      >Bagikan ke Tumblr</a
    >

    <a href="mailto:" class="share-button email" target="_blank"
      ><svg height="24" width="24" viewBox="0 0 24 24">
        <path
          fill="#FFFFFF"
          d="M20,8L12,13L4,8V6L12,11L20,6M20,4H4C2.89,4 2,4.89 2,6V18A2,2 0 0,0 4,20H20A2,2 0 0,0 22,18V6C22,4.89 21.1,4 20,4Z"
        /></svg
      >Kirim melalui Email</a
    >
  </div>
</template>

<script lang="ts" setup>
defineProps({
  url: {
    type: String,
    required: true,
  },
});
</script>
