...
// Get setFieldError from form context to clear errors
const formContext = inject<{ setFieldError?: (field: string, message: string | undefined) => void } | null>(
  "formContext",
  null,
);

// Control popover open state to ensure it can be opened after error
...
const handleChange = (v: DateValue | undefined) => {
  if (!v) {
    setValue("", false);
    // Clear error when clearing the field
    if (formContext?.setFieldError) {
      formContext.setFieldError(props.name, undefined);
    }
    return;
  }
  ...
  // Close popover after selecting date
  popoverOpen.value = false;
};

// Reset popover state when error occurs to ensure it can be opened again
watch(errorMessage, (newError) => {
  if (newError) {
    // Close popover if it's open when error occurs
    popoverOpen.value = false;
  }
});
</script>
          ...
          <Button
            ...
            :class="
              cn(
                ...
                errorMessage &&
                  '!border-destructive !ring-destructive focus-visible:ring-destructive hover:!border-destructive',
